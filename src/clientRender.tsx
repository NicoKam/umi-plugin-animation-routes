 ...
 * @param Routes
 */
export async function preloadComponent(readyRoutes: IRoute[], pathname = window.location.pathname): Promise<IRoute[]> {
...
export function renderClient(opts: IOpts) {
  ...
  if (opts.rootElement) {
    const rootElement =
      typeof opts.rootElement === 'string' ? document.getElementById(opts.rootElement) : opts.rootElement;
